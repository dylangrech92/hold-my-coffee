...
1. **Vertical slices only.** Each task ships observable behavior through the whole stack. Horizontal slicing ("all models, then all endpoints, then all UI") produces weeks of nothing-works and one unreviewable merge.
...
7. **Boundary contracts precede scoped tasks.** Work spanning a process/service/interface boundary gets the contract (protocol, data flow, daemon model) decided in writing before tasks are cut — implicit boundaries are how implementations come back wrong.

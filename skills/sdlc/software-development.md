...
1. **What executes at runtime** — in what order, with what data? Import graphs and text searches are not behavior.
...
- **Climb the ladder; stop at the first rung that holds.** Does it need to exist at all? → standard library? → platform/framework feature (a structural constraint or type over hand-rolled app code, declarative over imperative)? → already-installed dependency? → one line? → only then, minimum new code. Never add a dependency for what a few lines cover.
...
- **Contracts before tests.** A typed signature, an abstract method, a single return type, a DB constraint — each makes a bug class unrepresentable, which beats any test asserting the bug didn't happen. Interfaces are rigid: abstract methods force what genuinely varies; lock what must not vary with an immutability marker, whatever your language calls it. Drift enters through loose interfaces.
- **One authority for durable state.** In-memory state carried across calls, processes, or instances is a second source of truth that drifts. Rebuild it from the authoritative store (a database, a file, a persistent cache); any process may die at any line.
...
- **Loud errors.** Catch to add context and re-raise, or log with enough context to diagnose — one of the two, always. Exceptions for failures, never sentinel returns (null, -1, false-as-error). One return type per function.
...
- **Names are inherited, not invented.** Use the exact existing names — the owner's, the codebase's, the tool's. Classes are nouns, methods verbs, booleans read as questions (is-/has-/can- style, or your language's predicate convention). A new umbrella name for an existing thing is a translation layer everyone pays forever.
...
Committed tree (not working tree) verified: runs end-to-end, feature-tested, both branch sides covered, zero residue, any required data migration and fixtures shipped, docs updated, reviewed. Anything less is "in progress."
